...
if [ -f "$${IPKG_INSTROOT}/etc/pkt_fwd/global_conf.json" ]; then
		curts=$$(date +%y%m%d_%H_%M_%S)
        echo -e "\nBacking up GLOBAL settings files...\n"
        mv $${IPKG_INSTROOT}/etc/pkt_fwd/global_conf.json $${IPKG_INSTROOT}/etc/pkt_fwd/global_conf_$${curts}.json
fi

if [ -f "$${IPKG_INSTROOT}/etc/pkt_fwd/local_conf.json" ]; then
		curts=$$(date +%y%m%d_%H_%M_%S)
        echo -e "\nBacking up LOCAL settings files...\n"
        mv $${IPKG_INSTROOT}/etc/pkt_fwd/local_conf.json $${IPKG_INSTROOT}/etc/pkt_fwd/local_conf_$${curts}.json
fi
